...
class FileHandler:
    ...
    def sort_solution(self):
        try:
            if self.file:
                ...
                modified_content = self.remove_replace_all_text(content)
    ...
    def remove_replace_all_text(self,content):
        return [self.replace_text_with_number(line) + '\n' for line in content]

    def replace_text_with_number(self,text):
        for key in number_mapping:
            text = text.replace(key,key[0]+str(number_mapping[key])+key[-1])
        return text
